...
@:bitmap("assets/graphics/haxepunk.png")
...
class Preloader extends NMEPreloader
{
	...
	{
		tileData[4] += 1; // large cog
		tileData[9] += 1; // small cog
		tileData[13] += scaleIncrement; // factory scale
		if (tileData[13] > 1.02 || tileData[13] < 1)
		...
		boot.graphics.clear();
		tiles.drawTiles(boot.graphics, [0, 0, 0, 1, 0], true, Tilesheet.TILE_ROTATION | Tilesheet.TILE_SCALE);
	}
